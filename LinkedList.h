...
public:
	...
	node* find(int index) {
        node* current = NULL;
		if (indexInRange(index, 0, size/2)) { // closer to beginning
			current = head;
			for (int i = 0; i < index; i++) 
				if (current != NULL)
					current = current->next;
		}
		else if (indexInRange(index, (size/2 + 1), size)) { // closer to end
			current = tail;
    ...
    void insert(T value, int index) {
        ...
        else if (index == size) insertTail(value); // tail
        else { // middle
    ...
    T remove(int index) {
		...
		else if (index == size) throw out_of_range(""); // at the end
        else { // middle
            node* current = find(index);
